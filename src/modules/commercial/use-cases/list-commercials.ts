...
import CreateCommercialUseCase, { ICreateCommercialUseCase } from './create-commercial'
...
export default class ListCommercialsUseCase implements IListCommercialsUseCase {
  ...
  async execute(page: number, limit: number): Promise<CommercialsPaginated> {
    ...
    const data = await this.createCommercialUseCase.execute(pagination.skip, limit)
    return {
      ...pagination,
      data
    }
