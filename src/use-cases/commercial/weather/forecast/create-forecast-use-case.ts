...
import ForecastRepository, {
  IForecastRepository
} from '@repositories/commercial/forecast-repository'
...
export default class CreateForecastUseCase implements ICreateForecastUseCase {
  constructor(
    private repository: IForecastRepository = new ForecastRepository()
  ) {}

  async execute(weatherId: string, data: Prisma.ForecastCreateInput) {
    let alreadyExistsForecast = await this.repository.findByWeatherId(
      weatherId,
      data.index
    )
